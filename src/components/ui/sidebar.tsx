  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={twMerge(
          "fixed inset-y-0 z-10 hidden h-screen w-(--sidebar-width)",
          "backdrop-blur-xl bg-sidebar/80 dark:bg-bg/80",
          ...
          intent === "float" &&
            "bg-bg/80 p-2 group-data-[collapsible=dock]:w-[calc(--spacing(4)+2px)]",
          intent === "inset" &&
            "bg-sidebar/80 group-data-[collapsible=dock]:w-[calc(var(--sidebar-width-dock)+--spacing(2)+2px)] dark:bg-bg/80",
          intent === "default" && [
      ...
      >
        <div
          ...
          className={twJoin(
            "flex h-full w-full flex-col text-sidebar-fg overflow-hidden",
            "group-data-[intent=inset]:bg-transparent dark:group-data-[intent=inset]:bg-transparent",
            "group-data-[intent=float]:rounded-lg group-data-[intent=float]:border group-data-[intent=float]:border-sidebar-border group-data-[intent=float]:bg-sidebar/80 group-data-[intent=float]:shadow-xs",
          )}
        >
          {children}
        </div>
  ...
  return (
    ...
    >
      {state !== "collapsed" && "label" in props && (
        <Header className="mb-1 flex shrink-0 items-center gap-x-1.5 rounded-md px-2 font-medium text-sidebar-fg/70 text-xs/6 outline-none ring-sidebar-ring transition-[margin,opa] duration-200 ease-linear *:data-[slot=icon]:size-4 *:data-[slot=icon]:shrink-0 group-data-[collapsible=dock]:-mt-8 group-data-[collapsible=dock]:opacity-0">
          {props.label}
        </Header>
      )}
      <div
      ...
      className={composeRenderProps(
        ...
        (className, { isPressed, isFocusVisible, isHovered, isDisabled }) =>
          twMerge([
            ...
            "w-full min-w-0 items-center rounded-lg text-left font-medium text-sm/6 text-sidebar-fg",
...
const SidebarInset = ({ className, ref, ...props }: React.ComponentProps<"main">) => {
  return (
    <main
      ...
      ref={ref}
      className={twMerge(
        "relative flex w-full h-screen flex-1 flex-col bg-bg lg:min-w-0 overflow-y-auto",
    ...
    <Heading level={3}>
      <Trigger
        ...
        className={composeRenderProps(
          ...
          (className, { isPressed, isFocusVisible, isHovered, isDisabled }) =>
            twMerge(
              "flex w-full min-w-0 items-center rounded-lg text-left font-medium text-sm/6 text-sidebar-fg",
